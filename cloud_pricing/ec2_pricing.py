...
def dict2obj(dict_):
    return namedtuple('FilterObject', dict_.keys())(*dict_.values())


def get_price_on_demand_ec2_instance(filter_):
    result = dict(filter_)
    filter_['filter_type_pattern'] = None
    filter_['type'] = 'ondemand'

    error = ''
    try:
        data, _ = _get_data(dict2obj(filter_))
    except Exception, e:
        result['error'] = 'Format is wrong. Required parameters are `filter_os_type`, `filter_region`, `filter_type`, `type`.'
        return result        

    try:
        hourly = data['regions'][0]['instanceTypes'][0]['prices']['ondemand']['hourly']
    except Exception, e:
        result['error'] = 'There is no such instance. Please check parameters again.'
        return result

    result['Daily'] = "{0:.2f}".format(hourly * 24)
    result['Monthly'] = "{0:.2f}".format(hourly * 24 * 30.5)
...
if __name__ == "__main__":
    ...
    filter_ = {
        ...
        'filter_type': 'r3.large', 
        # 'type': 'ondemand'    
    }

    print (json.dumps(get_price_on_demand_ec2_instance(filter_)))
